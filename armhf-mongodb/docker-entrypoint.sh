...
#Test if command begins with mongo
if [[ $@ == mongo* ]]; then
	if [[ ${MONGO_OUT_FILTER_PATTERN:-} ]]; then
		echo "[MongoDB] Applying awk filter pattern to logs:$MONGO_OUT_FILTER_PATTERN"
		exec "$@" | mawk -Winteractive -v pattern="$MONGO_OUT_FILTER_PATTERN" '$0 !~ pattern {print "[MongoDB] " $0}'
	else
		exec "$@" | mawk -Winteractive '{print "[MongoDB] " $0}'
	fi
else
	exec "$@"
fi
